...
<body>
	...
	<form action="/HIS/admin/editPatient.do" id="myform" class="myform" method="post" onsubmit="return check()">
	<input type="hidden" name="code" value="${patient.code }">
	<input type="hidden" name="id" value="${patient.id }">
	<div class="main" id="main">
		<div class="main_bar">
			<button type="button" onclick="modify()">修改</button>
			<a href="/HIS/admin/patientList.do" class="arrow-left_a faz" title="返回病人列表"><i class="fa fa-arrow-left fa-2x faz" aria-hidden="true"></i></a>
			...
			<table>
				<tr>
					<td class="td_4">姓名</td>
					<td><input type="text" class="td_3" name="name" value="${patient.name }" id="name"></td>
					<td class="td_4">性别</td>
					<td>
					...
					<td class="td_4">年龄</td>
					<td><input type="text" class="td_3" name="age" value="${patient.age }" id="age"></td>
				</tr>
				<tr>
					<td class="td_4">联系电话</td>
					<td><input type="text" class="td_3" name="phone" value="${patient.phone }" id="phone"></td>
					...
					<td>
					<input type="text" class="td_1 td_3 td_3_gdd" value="<c:if test="${patient.status == 1 }">在医</c:if><c:if test="${patient.status == 2 }">住院</c:if><c:if test="${patient.status == 3 }">出院</c:if>">
					<select name="status" class="td_3 td_3_insurance_type">
						<option value="1" <c:if test="${patient.status == 1 }">selected="selected"</c:if>>在医</option>
						<option value="2" <c:if test="${patient.status == 2 }">selected="selected"</c:if>>住院</option>
						<option value="3" <c:if test="${patient.status == 3 }">selected="selected"</c:if>>出院</option>
					</select>
				</tr>
				<tr>
